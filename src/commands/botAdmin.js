...
function setGame(bot) {
  if (!(bot.user.presence.game && bot.user.presence.game.url)) {
      ...
      "Brawlball", "Snowbrawl", "Dodgebomb", "Bombsketball",
      "Beachbrawl", "Switchcraft", "Kung Foot", "Horde",
	    "Training"
		];
...
function reload(files) {
  ...
  let Utils = require(path.resolve(process.cwd(), "./utils/utils"));

  files.forEach(file => {
    Utils.handler.reload(path.resolve(process.cwd(), "./commands/", file));
  });
}
...
.addCommand({name: "botping",
	description: "Check bot ping.",
  category: "Bot Admin", hidden: true,
	process: (msg) => {
		msg.channel.send('Pinging...').then(sent => {
  		sent.edit(`Pong! Took ${sent.createdTimestamp - (msg.editedTimestamp ? msg.editedTimestamp : msg.createdTimestamp)}ms${msg.client.shard ? " on shard " + msg.client.shard.id : ""}`);
		});
	}
})
...
.addCommand({name: "pulse",
  ...
  process: async function(msg, suffix) {
    ...
    if (bot.shard) {
      ...
      let channels = await bot.shard.fetchClientValues('channels.size');
      channels = channels.reduce((prev, val) => prev + val, 0);
      let mem = await bot.shard.broadcastEval("Math.round(process.memoryUsage().rss / 1024 / 1000)");
  ...
  process: async function(msg, shard) {
    try {
      ...
      if (msg.client.shard) {
        ...
        if (shardId && msg.client.shard.id == shardId) {
          await msg.client.destroy();
          process.exit();
        } else if (shardId) {
          msg.client.shard.broadcastEval(`if (this.shard.id == ${shardId}) this.destroy().then(() => process.exit())`)
...
.addCommand({name: "reload",
  ...
  process: async (msg, suffix) => {
    ...
    if (!msg.client.shard) {
      ...
      try {
        await msg.client.shard.broadcastEval(`(${reload})([${files.map(file => `"${file}"`).join()}])`)
      } catch(e) { u.alertError(e, msg); }
    }
    await msg.react("👌");
  },
...
.setInit((reload = false) => {
  u.errorLog.send(u.embed().setTimestamp().setDescription((Module.handler.client.shard ? ("Shard " + Module.handler.client.shard.id) : "Bot") + " is ready!"));
})
.setUnload(() => true)
.setClockwork(() => {
