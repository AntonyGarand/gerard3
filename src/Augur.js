/**
 * I had to create a local copy of Augur so it used the same version of discord.js as the Gerard project
 * Based off 1.2.0
 */
...
const Handler = function (config, options = {}) {
  ...
  this.config = config;
  this.db =
    this.config.db && this.config.db.model
      ? require(path.resolve(process.cwd(), this.config.db.model))
      : null;
...
Handler.prototype.parse = async function (msg) {
  try {
    if (this.options.parse) return await this.options.parse(msg);
      ...
      else if (message.startsWith(`<@${msg.client.user.sid}>`))
        parse = `<@${msg.client.user.id}>`.length;
      ...
      if (load.events && load.events.size > 0) {
        load.events.forEach((handler, event) => {
          if (!this.events.has(event))
            this.events.set(event, [{ file: file, handler: handler }]);
...
Handler.prototype.start = function () {
  return new Promise((fulfill, reject) => {
    ...
    Client.on("ready", () => {
      ...
      console.log(
        `Listening to ${Client.channels.cache.size} channels in ${Client.guilds.cache.size} servers.`
      );
      ...
      try {
        ...
        if (this.events.has("messageUpdate")) {
          for (let i = 0; i < this.events.get("messageUpdate").length; i++) {
            if (
              (halt = await this.events
                .get("messageUpdate")
                [i].handler(oldMsg, msg))
            )
              break;
          }
        }
